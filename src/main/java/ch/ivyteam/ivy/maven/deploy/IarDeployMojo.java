...
public class IarDeployMojo extends AbstractEngineMojo {
  ...
  /**
   * The IAR to deploy. By default the packed IAR from the
   * {@link IarPackagingMojo#GOAL} is used.
   */
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    getLog().warn("The goal '" + GOAL + "' is deprecated! Please migrate to the new goal '"
            + DeployToEngineMojo.GOAL + "'.");
    ...
    if (!deployDir.exists()) {
      getLog().warn("Skipping IAR deployment to engine '" + deployEngineDirectory + "'. The directory '"
    ...
    IvyDeployer deployer = new FileDeployer(deployDir, null, deployTimeoutInSeconds, deployIarFile,
            targetIarFile);
    deployer.deploy(iarPath, getLog());
  }

  private File getDeployDirectory() throws MojoExecutionException {
    if (deployEngineDirectory == null) { // re-use engine used to build
